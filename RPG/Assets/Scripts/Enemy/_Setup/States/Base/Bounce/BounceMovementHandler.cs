...
public class BounceMovementHandler<TEnemy> where TEnemy : Enemy_Regular
{
    ...
    private void MoveAirborneArc()
    {
        ...
        {
            airStartPosition = state.MovementRigidbody.position;
            airTravelDistance = 0f;
    ...
    private void MoveAirborneCurveFromEdge()
    {
        ...
        if (Mathf.Abs(xDirection) <= 0.0001f)
            xDirection = state.MovementPlannedJumpSide == BounceStateBase<TEnemy>.BounceJumpSide.Right ? 1f : -1f;
        ...
        float totalDistance = Mathf.Max(0.001f, state.MovementPlannedJumpDistance);
        float targetBend = Mathf.Max(0.001f, state.MovementGetFloorToCeilingSpan());
    ...
    private void StoreEdgeLaunchDirection(Vector2 beforeLaunchPosition, Vector2 afterLaunchPosition, Vector2 fallbackDirection)
    {
        ...
        float originalDistance = Mathf.Max(0.001f, state.MovementPlannedJumpDistance);
    ...
    private void MoveAirborneCurveFromFloor()
    {
        ...
        if (!curveReachedEnd)
        {
            ...
            if (Mathf.Abs(xDirection) <= 0.0001f)
                xDirection = state.MovementPlannedJumpDirection.x >= 0f ? 1f : -1f;
        ...
        Vector2 continuePosition = state.MovementRigidbody.position + upDirection * (MoveSpeed() * deltaTime);
    ...
    private void MoveAirborneCurveFromWall()
    {
        ...
        float verticalDistance = Mathf.Max(0.001f, state.MovementGetFloorToCeilingSpan());

        float verticalDirection = Mathf.Sign(state.MovementPlannedJumpDirection.y);
        if (Mathf.Abs(verticalDirection) <= 0.0001f)
            verticalDirection = state.MovementPlannedJumpSide == BounceStateBase<TEnemy>.BounceJumpSide.Right ? 1f : -1f;

        float outwardDirection = state.MovementCurrentSurface == BounceStateBase<TEnemy>.BounceSurface.RightWall ? -1f : 1f;
        float outwardDistance = Mathf.Max(0.001f, state.MovementPlannedJumpDistance);
    ...
    private void MoveAirborneCurveFromCeiling()
    {
        ...
        float fallX = state.MovementPlannedJumpDirection.x;
        if (Mathf.Abs(fallX) <= 0.0001f)
            fallX = state.MovementPlannedJumpSide == BounceStateBase<TEnemy>.BounceJumpSide.Right ? 1f : -1f;

        Vector2 fallDirection = new Vector2(fallX * 0.35f, -1f).normalized;
        Vector2 continuePosition = state.MovementRigidbody.position + (fallDirection * (MoveSpeed() * deltaTime));

        MoveAirborneCurveTravel(continuePosition, fallDirection);
    }

    #region Helpers
    private void MoveAirborneCurveTravel(Vector2 targetPosition, Vector2 pathDirection)
    {
        ApplyBounceMovement(targetPosition, pathDirection);
        RotateAirborne(pathDirection);
    }
